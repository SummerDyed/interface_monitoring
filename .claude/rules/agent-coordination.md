...
Each stream maintains progress in `.claude/epics/{epic}/updates/{issue}/stream-{X}.md`:
```markdown
---
...
The analysis file is the contract. Agents read this to understand boundaries:
```yaml
Stream A:
...
### Conflict Resolution
Always defer to humans. Never attempt automatic merge resolution.

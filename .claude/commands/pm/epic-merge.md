...
2. **Check current branch:**
   ...
   if [[ "$current_branch" != "epic/$ARGUMENTS" ]]; then
     echo "⚠️ You're not on the epic branch. Switch first: git checkout epic/$ARGUMENTS"
   fi
   ```
...
3. Get help reviewing conflicts:
   git diff --name-only --diff-filter=U
"
exit 1
```
